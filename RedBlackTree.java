...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    // Rotate Left
    if (!child.isLeftChild() && parent.rightChild == child) {
      // Shift subtrees
      parent.rightChild = child.leftChild;
      child.parent = parent.parent;
      // Check if parent is root
      if (parent.parent != null) {
        // Check if parent is left child and rotates the child into position
    ...
    // Rotate Right
      ...
      // Check if parent is root
      if (parent.parent != null) {
        // Check if parent is left child and rotates child into position
        if (parent.isLeftChild()) {
          parent.parent.leftChild = child;
        } else {
          parent.parent.rightChild = child;
    ...
    // Throws exception if invalid arguments are inputed
    else {
   ...
   * traversal. The toString of the Node class class above produces a level order traversal of the
   * nodes / values of the tree.
   * 
  ...
  /**
   * This is a private recursive helper method that will be called in the insertHelper method which
   * takes a reference to the newly added red node as a parameter. This method resolves the red
   * child under a red parent red black tree property violations that are introduced when inserting
   * a new node into the tree
   *  
   * @param n - the node that this method checks
   */
  private void enforceRBTreePropertiesAfterInsert(Node<T> n) {
    // Checks if parent is null and if the parent is the root
    if (n.parent != null && !n.parent.equals(root)) {
      Node<T> uncle = null;
      Node<T> parent = n.parent;
      Node<T> grandParent = n.parent.parent;
      // Initializes the uncle node
      if (n.parent.isLeftChild()) {
        uncle = grandParent.rightChild;
      } else {
        uncle = grandParent.leftChild;
      }
      // Case 1: Uncle is black, Parent is red, Both parent and child are right children
      if (n.isLeftChild() && parent.isLeftChild() && !parent.isBlack
      ...
      // Case 1: Uncle is black, Parent is red, Both parent and child are right children
      else if (!n.isLeftChild() && !parent.isLeftChild() && !parent.isBlack
          && (uncle == null || uncle.isBlack)) {
        parent.isBlack = !parent.isBlack;
        grandParent.isBlack = !grandParent.isBlack;
        rotate(parent, grandParent);
      }
      // Case 2: Uncle is black, Parent is red, Parent and child are on different sides
      if ((parent.isLeftChild() && !n.isLeftChild())
          || (!parent.isLeftChild() && n.isLeftChild())) {
        if (!parent.isBlack && (uncle == null || uncle.isBlack)) {
          // Rotate to make case 1 and recursive call to solve case 1
          Node<T> temp = parent;
          rotate(n, parent);
      ...
      // Case 3: Uncle is Red
      if (uncle != null && !parent.isBlack && !uncle.isBlack) {
        // Color swap and recursive call to check for any violations
        parent.isBlack = !parent.isBlack();
